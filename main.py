...
def resource_path(relative_path):
    """Trova il percorso corretto sia in sviluppo che nell'exe compilato con PyInstaller."""
    try:
...
engine_path = resource_path('sounds/engine.wav')
if os.path.exists(engine_path):
    try:
        engine_sound = Audio(engine_path, loop=True, autoplay=True, volume=0.3)
        if engine_sound.clip is None:
            engine_sound = None
    except:
        engine_sound = None

skid_path = resource_path('sounds/skid.wav')
if os.path.exists(skid_path):
    try:
        skid_sound = Audio(skid_path, loop=False, autoplay=False, volume=0.5)
        if skid_sound.clip is None:
            skid_sound = None
    except:
        skid_sound = None
...
def update():
    ...
    if game_over:
        if engine_sound is not None and engine_sound.clip is not None:
    ...
    if skid_sound is not None and skid_sound.clip is not None and turning and abs(car_speed) > max_speed * 0.6 and skid_cooldown <= 0:
        skid_sound.play()
        skid_cooldown = 0.8
